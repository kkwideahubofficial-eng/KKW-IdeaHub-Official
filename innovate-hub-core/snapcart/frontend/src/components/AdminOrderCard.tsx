...
function AdminOrderCard({ order }: { order: IOrder }) {
    console.log("AdminOrderCard rendered for order status:", order.status);
        ...
        >
            <div className='flex flex-col md:flex-row md:items-start md:justify-between gap-4'>
              ...
              {order.assignedDeliveryBoy && (
                <div className='mt-4 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-100 rounded-xl p-4 flex items-center justify-between shadow-sm'>
                    <div className='flex items-center gap-4'>
                        <div className="w-10 h-10 bg-blue-200 rounded-full flex items-center justify-center text-blue-700 font-bold">
                            {order.assignedDeliveryBoy.name?.charAt(0) || "D"}
                        </div>
                        <div>
                            <p className='text-xs text-blue-600 font-semibold uppercase tracking-wider'>Assigned Driver</p>
                            <p className='font-bold text-gray-800 text-sm'>{order.assignedDeliveryBoy.name}</p>
                            <p className='text-xs text-gray-600 flex items-center gap-1'>
                                <Phone size={12}/> +91 {order.assignedDeliveryBoy.mobile}
                            </p>
                        </div>
                    </div>
                    <a 
                        href={`tel:${order.assignedDeliveryBoy.mobile}`} 
                        className='bg-blue-600 text-white text-xs px-4 py-2 rounded-lg hover:bg-blue-700 transition shadow-blue-200 shadow-lg font-medium'
                    >
                        Call Driver
                    </a>
                </div>
